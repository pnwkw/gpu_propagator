...
viz::glsl::glsl() {
	...
	gl::glNamedBufferData(sol_segments_ssbo, sizeof(mag_field::mag_cheb::Sol_Segments), mag->getSolSegmentsPtr(), gl::GL_STATIC_COPY);
	gl::glNamedBufferData(sol_params_ssbo, sizeof(mag_field::mag_cheb::Sol_Params), mag->getSolParamsPtr(), gl::GL_STATIC_COPY);

	gl::glNamedBufferData(dip_segments_ssbo, sizeof(mag_field::mag_cheb::Dip_Segments), mag->getDipSegmentsPtr(), gl::GL_STATIC_COPY);
	gl::glNamedBufferData(dip_params_ssbo, sizeof(mag_field::mag_cheb::Dip_Params), mag->getDipParamsPtr(), gl::GL_STATIC_COPY);
}
